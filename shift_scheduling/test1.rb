...
class Schedule
  ...
  def     
    @scheduled_shifts, @employees_list, @shifts = [[], [], []]
  end
  ...
  def add_new_shift
    ...
    loop do
      ...
      begin
        case gets.strip
        when 'Y', 'y', 'yes'
          shift_name, start_time, end_time = get_shift_details_from_user
          is_overlapped = false
  ...
  # Get valid inputs from user for shift
  def get_shift_details_from_user
  ...
  def get_employee_details(available_employees, shift_st, shift_et, assigned_employees)
    ...
    puts "Please enter End Time in between #{start_time} to #{shift_et}"
    puts 'And Employee worked hours should not be greater then 10'
    end_time = gets.chomp
