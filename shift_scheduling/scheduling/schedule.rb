...
class Shift
  ...
  def worker_schedule(worker)
    worker.availablities.each do |work_availibilty|
      break if worker.total_scheduled_hours >= 40
      ...
      unless !comman_range.nil? && comman_range.max != comman_range.min && ((worker.total_scheduled_hours + (comman_range.max - comman_range.min)) <= 40)
        next
      end
...
class WorkerAvailability
  ...
  attr_accessor :day, :available_from, :till_available, :available, :consumed_hours, :remaining_hours, :shift_hours,
...
class Restuarent
  ...
  def dummy_shifts
    ...
    worker.initialize_dummy({ name: 'mohan', current_positions: %w[m f], availablities: [
                              { day: '0', available: 'Y', start_t: [10, 21] },
                              { day: '1', available: 'Y', start_t: [6, 12] },
                              { day: '2', available: 'n', start_t: [20, 24] },
                              { day: '3', available: 'n', start_t: [0, 8] },
    ...
    worker = Worker.new(@workers.length)
    worker.initialize_dummy({ name: 'radhe', current_positions: %w[m f], availablities: [
