...
module Data_store
  def data
    # $shifts = {
    # 						0 => ["night",[0,8], {'cook' => 2,'shipper'=>1}],
    # 					 	1 => ["morning",[8,16], {'cook' => 2,'shipper'=>1}],
    # 					 	2 => ["evening",[16,24], {'cook' => 2,'shipper'=>1}]
    # 					}
    ...
    worker = Worker.new(@workers.length, 1)
    worker.initialize_dummy({ name: 'use2', current_positions: %w[cook shipper], availablities: [
                              { day: '0', available: 'Y', start_t: [8, 10] },
    ...
    worker = Worker.new(@workers.length, 1)
    worker.initialize_dummy({ name: 'user2', current_positions: ['shipper'], availablities: [
                              { day: '1', available: 'Y', start_t: [8, 10] },
                              { day: '0', available: 'n', start_t: [9, 18] },
                              { day: '2', available: 'n', start_t: [9, 15] },
                              { day: '3', available: 'n', start_t: [9, 18] },
                              { day: '4', available: 'n', start_t: [9, 15] },
                              { day: '5', available: 'n', start_t: [9, 15] },
    ...
    worker.initialize_dummy({ name: 'user3', current_positions: ['shipper'], availablities: [
                              { day: '1', available: 'Y', start_t: [12, 14] },
                              { day: '0', available: 'n', start_t: [9, 18] },
                              { day: '2', available: 'n', start_t: [9, 15] },
                              { day: '3', available: 'n', start_t: [9, 18] },
                              { day: '4', available: 'n', start_t: [9, 15] },
    ...
    worker.initialize_dummy({ name: 'user4', current_positions: ['shipper'], availablities: [
                              { day: '1', available: 'Y', start_t: [8, 12] },
                              { day: '0', available: 'n', start_t: [9, 18] },
                              { day: '2', available: 'n', start_t: [9, 15] },
                              { day: '3', available: 'n', start_t: [9, 18] },
                              { day: '4', available: 'n', start_t: [9, 15] },
...
class Shift
  ...
  def worker_schedule(worker, day)
    ...
    # byebug if worker.worker_id == "RSI1" || worker.worker_id == "RSI4"
    ...
    return if work_availibilty.assign_shift_hours(comman_range).include?(0..8)
...
class WorkerAvailability
  ...
  def check_shift_hours(common_range, previous_work, all_vacant_schedules)
    ...
    result << find_max_hour_schedule_range(common_range, all_vacant_schedules)
    byebug
    result
  end
  ...
  def find_max_hour_schedule_range(range, all_vacant_schedules)
    ...
    all_vacant_schedules.each do |schedule|
      schedule.vacant_hours.each do |vacant_range|
        if (vacant_range & range).difference > max_time_range
...
class Restuarent
  ...
  # def dummy_shifts
  # 	@shifts << Shift.new(@shifts.length).initialize_dummy(["night",[0,8], {'cook'=>2,'shipper'=>1}])
  ...
  # 	worker = Worker.new(@workers.length, 1)
  # 	worker.initialize_dummy({name: "user1", current_positions: ['cook','shipper'], availablities: [
  ...
  # 	worker = Worker.new(@workers.length, 1)
  # 	worker.initialize_dummy({name: "user2", current_positions: ['cook','shipper'], availablities: [
