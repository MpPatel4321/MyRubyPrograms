...
class Company
  ...
  # $employee = { 'name' => "Mahesh", 'employee_id' => "EMP0", 'position' => "developer" , "timming" => {"Sunday" => [0,24], 'Monday' => [13,24], 'Tuesday' => [10,24], 'Wednesday' => [], 'Thursday' => [0,16], 'Friday' => [0,20], 'Saturday' => [0,20] }}
  # $employees << $employee
  # # $shift_scheduling = { "emp_name" => "Mahesh", 'day' => "Sunday", 'shift_id' => 0 ,'shift_name' => "morning", 'timming' => [[0,8]] ,'position' => "developer" }
  # # $shifts_scheduling << $shift_scheduling
  # # $shift_scheduling = { "emp_name" => "Mahesh", 'day' => "Monday", 'shift_id' => 1 ,'shift_name' => "evenning", 'timming' => [[8,10]],'position' => "developer" }
  # # $shifts_scheduling << $shift_scheduling
  # # $shift_scheduling = { "emp_name" => "M", 'day' => "Sunday", 'shift_id' => 0 ,'shift_name' => "morning", 'timming' => [[0,8]] ,'position' => "tester" }
  # # $shifts_scheduling << $shift_scheduling
  # $shift = { 'shift_name' => "morning", 'shift_id' => 0, 'timming' => [0,8] }
  # $shifts << $shift
  # $shift = { 'shift_name' => "afternoon", 'shift_id' => 1, 'timming' => [8,16] }
  # $shifts << $shift
  # $shift = { 'shift_name' => "evenning", 'shift_id' => 2, 'timming' => [16,24] }
  # $shifts << $shift
    ...
    $days.each do |_key, value|
      ...
      if %w[Y y].include?(yes)
        ...
        if employee_hash['timming'][value.to_s].length <= 1
          ...
          redo
          elseif(employee_hash['timming'][value.to_s].max.zero? || (employee_hash['timming'][value.to_s].max > 24))
          puts 'You enter wrong timming ......'
          puts 'Please enter time in range of 0-24'
          redo
        end
  ...
  def add_shift
    ...
    loop do
      ...
      if shift_hash['timming'].length <= 1
        ...
        puts 'Please enter time in range of 0-24'
        redo
      elsif shift_hash['timming'].max.zero? || shift_hash['timming'].max > 24
        puts 'You enter wrong timming ......'
        puts 'Please enter time in range of 0-24'
        redo
      else
        check_timming = false
        (0...$shifts.length).each do |_i|
          shift_min = $shifts[0]['timming'].min
          shift_max = $shifts[0]['timming'].max
          check_timming = true if shift_min < shift_hash['timming'].min && shift_hash['timming'].min < shift_max
        end
        shift = $shifts.detect { |value| shift_hash['timming'] == value['timming'] }
  ...
  def find_shift
    shift = []
    puts 'Enter Shift id'
    @shiftid = gets.chomp.to_i

    shift = $shifts.detect { |value| @shiftid == value['shift_id'].to_i }
    if shift.nil?
  ...
  def show_detail(shift)
    ...
    $days.each do |_key, value|
      ...
      shift_days  = $shifts_scheduling.select { |days| value == days['day'] && @shiftid == days['shift_id'] }
  ...
  def scheduling(employee_hash)
    ...
    availablities.each do |day, value|
      ...
      (0...$shifts.length).each do |i|
        next unless total_hours <= 40 || total_day_hours <= 10
        ...
        puts "#{day} = #{shift_start} - #{shift_end}" if shift_start != shift_end
      end
    end
    puts total_hours
    add_show_employee
